...
std::vector<LaneLine> LineDetection::detectLines(cv::Mat binaryEdgeImage, cv::Mat& debugImage, int& rawLineCount) {
    std::vector<cv::Vec2f> rawLines;

    // 1. Hough Transform
    cv::HoughLines(binaryEdgeImage, rawLines, HOUGH_RHO_RES, HOUGH_THETA_RES, m_houghThreshold);

    // --- NEU: ANZAHL SPEICHERN ---
    rawLineCount = (int)rawLines.size();
    // -----------------------------
